...
- GPS를 통하여 얻은 사용자 위치와 
  사용자 정보를 비콘에 전달함.
- 위치 정보와 사용자 정보를 전달   받은 비콘은 다시 서버로 전송
...
##4-2. 시스템 설계 과정의 실험
    ...
    - 시스템/서비스의 작동 여부
     ...
     : 서버가 시스템 과부하 시에도 얼마나 오랫동안 정상적으로 작동할 수 있는지 파악
...
"보행 중 스마트폰 사용"을 막는 어플로, 키와 걷는 방식, 속도에 따라 움직임을 
 3단계로 등록한다. 걸으면 (금지 앱이) 작동하고 전철을 타면 작동하지 않는 어플이다.
