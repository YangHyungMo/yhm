...
> `보행 중 스마트폰 사용"을 막는 어플로, 키와 걷는 방식, 속도에 따라 움직임을 3단계로 등록한다. 걸으면 (금지 앱이) 작동하고 전철을 타면 > 작동하지 않는 어플이다. 이 어플리케이션도 스마트폰 보행 사고를 막기 위해 나온 어플이지만, 보행 중에 스마트폰을 사용하지 않는다는 > > > 거추장스러움을 사용자에게 유발할 수 있기에, 우리 시스템이 더 효율적으로 사용될 수 있다고 본다. ` 
------------------------------------
